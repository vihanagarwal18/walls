...
class _FullScreenImageState extends ConsumerState<FullScreenImage> {
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect( // Add ClipRRect here
      borderRadius: BorderRadius.circular(14.0), // Set the border radius
      child:  Container(
        width: width * 0.80, //double.infinity,
        height: (16 * width * 0.79639) / 9 + 57.533298, //(16 * width * .60) / 9,
        child: Scaffold(
          body: Container(
  ...
  Widget _buildLikeButton(bool isLiked) {
    return LikeButton(
      ...
      isLiked: isLiked,
      onTap: (bool isLiked) async {
        ref.read(likeStateProvider(widget.imageName).notifier).toggleLike();
        return !isLiked;
      },
      likeBuilder: (bool isLiked) {
        return Icon(
